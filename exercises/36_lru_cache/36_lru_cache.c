...
static unsigned hash_int(int key) {
    return (unsigned)((key ^ (key >> 16)) * 2654435761U);
}
...
static LRUCache* lru_create(int capacity) {
    ...
    c->head = c->tail = NULL;
    c->bucket_count = 16;
    c->buckets = (HashEntry**)calloc(16, sizeof(HashEntry*));
    return c;
}
...
static void lru_put(LRUCache* c, int key, int value) {
    ...
    HashEntry* e = hash_find(c, key, &pentry);
    if (e) {
        e->node->value = value;
        list_move_to_head(c, e->node);
        return;
    ...
    if (c->size >= c->capacity && c->tail) {
        ...
        size_t idx = hash_int(removed->key) % c->bucket_count;
        HashEntry** p = &c->buckets[idx];
        while (*p && (*p)->node != removed) p = &(*p)->next;
        if (*p) {
            HashEntry* temp = *p;
            *p = (*p)->next;
            free(temp);
