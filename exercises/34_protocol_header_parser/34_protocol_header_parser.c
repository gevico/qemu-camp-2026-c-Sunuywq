 ...
 */
typedef struct {
    uint16_t version_raw;    // 2字节版本号（大端格式）
    uint16_t length_be;      // 2字节长度（大端格式）
    uint8_t flags_raw;       // 1字节标志
} proto_header_raw_t;
 ...
 */
typedef struct {
    uint8_t ver_minor : 4;   // 次版本（4位）
    uint8_t ver_major : 4;   // 主版本（4位）
    uint16_t length;          // 长度（16位）
    uint8_t flags : 5;        // 功能标志（5位）
...
int main(void) {
    ...
    uint16_t ver_be = be16_to_cpu(raw.version_raw);
    unsigned ver_major = (ver_be >> 4) & 0xF;
    unsigned ver_minor = ver_be & 0xF;
